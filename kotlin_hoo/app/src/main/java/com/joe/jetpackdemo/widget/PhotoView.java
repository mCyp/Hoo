...
public class PhotoView extends AppCompatImageView {
    ...
    private void init() {
        attacher = new CustomAttacher(this);
        ...
        ((CustomAttacher)attacher).setOnViewFingerUpListener(() -> {
            // 这里恢复位置和透明度
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ViewParent parent = getParent();
                if (parent instanceof ViewGroup && ((ViewGroup)parent).getBackground().mutate().getAlpha() <= 100 && mExitListener != null) {
                    exit();
                } else {
                    ...
                    va.addUpdateListener(animation -> PhotoView.this.setAlpha((Float) animation.getAnimatedValue()));
                    va.start();

                    bgVa.addUpdateListener(animation -> getRootView().getBackground().setAlpha((Integer) animation.getAnimatedValue()));
                    bgVa.start();
