package com.joe.jetpackdemo.ui;
...
import com.joe.jetpackdemo.widget.PhotoView;
...
public class CusChangeOnlineImageTransition extends Transition {
    ...
    private void captureBoundsAndInfo(TransitionValues transitionValues) {
        ...
        if (imageView.getScaleType() == ImageView.ScaleType.MATRIX || imageView instanceof PhotoView) {
    ...
    protected void calculateMatrix(TransitionValues startValues, TransitionValues endValues, int imageWidth, int imageHeight, Matrix startMatrix, Matrix endMatrix) {
        ...
        if (startScaleType == ImageView.ScaleType.MATRIX || (startValues.view instanceof PhotoView && (float)startValues.values.get(PROPNAME_SCALE) != 1f)) {
            Matrix matrix = null;
            matrix = (Matrix) startValues.values.get(PROPNAME_MATRIX);
